 ...
 * Displays transfer transactions with filtering, summary stats, and new transfer creation.
...
function buildSummaryCards(
  ...
  const todaysTransfers = rows.filter((r) => r.date === today).length;
  const totalValue = rows.reduce((s, r) => s + r.totalCost, 0);

  const cards = [
    { label: 'Total Transfers', value: String(totalTransfers) },
    { label: "Today's Transfers", value: String(todaysTransfers) },
...
function buildTable(rows: TransferRow[]): HTMLElement {
  const wrap = el(
    'div',
  ...
  for (const row of rows) {
    ...
    tr.appendChild(el('td', 'py-2 px-3 text-[var(--text-muted)]', row.fromWarehouseName));
    tr.appendChild(el('td', 'py-2 px-3 text-[var(--text-muted)]', row.toWarehouseName));
    tr.appendChild(
...
async function loadAndRender(): Promise<void> {
  ...
  const filters: { type: 'transfer'; dateFrom?: string; dateTo?: string } = {
    type: 'transfer',
  };
  ...
  let rows: TransferRow[] = transactions.map((t) => ({
    id: (t as any).id as string,
    date: t.date,
    itemName: itemMap.get(t.itemId) ?? t.itemId,
    fromWarehouseName: warehouseMap.get(t.warehouseId) ?? t.warehouseId,
    toWarehouseName: warehouseMap.get(t.toWarehouseId ?? '') ?? (t.toWarehouseId || '\u2014'),
    quantity: t.quantity,
    unitCost: t.unitCost,
...
async function refresh(): Promise<void> {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to load transfer transactions';
...
void (async () => {
  try {
    ...
    headerRow.appendChild(
      el('h1', 'text-2xl font-bold text-[var(--text)]', 'Inventory Transfers'),
    );
    const newBtn = el(
      'button',
    ...
    const message = err instanceof Error ? err.message : 'Failed to load inventory transfers';
    showMsg(wrapper, message, true);
  }
})();
...
async function handleNewTransfer(): Promise<void> {
  try {
    const svc = getInventoryService();
    ...
    const fromWarehouseId = prompt('From Warehouse ID:');
    if (!fromWarehouseId) return;

    const toWarehouseId = prompt('To Warehouse ID:');
    ...
    showMsg(wrapper, 'Transfer recorded successfully.', false);
    await loadLookups();
    await refresh();
  } catch (err: unknown) {
