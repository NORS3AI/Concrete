/**
 * Daily Field Logs view.
 * Displays daily logs with job, date, weather, crew count, work performed, delays, and safety notes.
...
export default {
  render(container: HTMLElement): void {
    ...
    titleWrap.appendChild(el('h1', 'text-2xl font-bold text-[var(--text)]', 'Daily Field Logs'));
    ...
    loading.appendChild(el('span', 'text-sm text-[var(--text-muted)]', 'Loading daily logs...'));
    ...
    async function loadTable(): Promise<void> {
      ...
      let records = await service.listDailyLogs(filters as any);
    ...
    function renderTable(records: any[]): void {
      ...
      if (records.length === 0) {
        ...
        empty.appendChild(el('p', 'text-[var(--text-muted)] text-sm', 'No daily logs found. Field logs will appear here once submitted from the field.'));
      ...
      for (const col of ['Job', 'Date', 'Weather', 'Crew Count', 'Work Performed', 'Delays', 'Safety Notes', 'Status']) {
      ...
      for (const row of records) {
        ...
        tr.appendChild(el('td', tdCls, row.weather || '--'));
        tr.appendChild(el('td', tdCls + ' font-mono text-right', String(row.crewCount)));

        // Work performed - truncate if long
        const workText = row.workPerformed.length > 60
          ? row.workPerformed.substring(0, 60) + '...'
          : row.workPerformed;
        tr.appendChild(el('td', tdCls + ' max-w-xs', workText));

        tr.appendChild(el('td', tdCls + ' text-[var(--text-muted)]', row.delays || '--'));
        tr.appendChild(el('td', tdCls + ' text-[var(--text-muted)]', row.safetyNotes || '--'));
    ...
    void (async () => {
      try {
        ...
        const message = err instanceof Error ? err.message : 'Failed to load daily logs.';
