/**
 * Report Templates view.
 * Manages saved report templates allowing users to create, view, edit,
 * and delete templates for quick access to commonly used report configurations.
 * Wired to ReportsService.
 */

import { getReportsService } from '../service-accessor';

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function el<K extends keyof HTMLElementTagNameMap>(
  tag: K,
  cls?: string,
  text?: string,
): HTMLElementTagNameMap[K] {
  const node = document.createElement(tag);
  if (cls) node.className = cls;
  if (text !== undefined) node.textContent = text;
  return node;
}

function showMsg(container: HTMLElement, text: string, isError: boolean): void {
  const existing = container.querySelector('[data-msg]');
  if (existing) existing.remove();
  const cls = isError
    ? 'p-3 mb-4 rounded-md text-sm bg-red-500/10 text-red-400 border border-red-500/20'
    : 'p-3 mb-4 rounded-md text-sm bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
  const msg = el('div', cls, text);
  msg.setAttribute('data-msg', '1');
  container.prepend(msg);
  setTimeout(() => msg.remove(), 5000);
}

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

const REPORT_TYPES = [
  'balance-sheet',
  'income-statement',
  'cash-flow',
  'wip-schedule',
  'job-cost-summary',
  'job-cost-detail',
  'aging-ap',
  'aging-ar',
  'payroll-summary',
  'payroll-detail',
  'equipment-utilization',
  'equipment-cost',
  'bonding-capacity',
  'custom',
] as const;

const REPORT_TYPE_LABELS: Record<string, string> = {
  'balance-sheet': 'Balance Sheet',
  'income-statement': 'Income Statement',
  'cash-flow': 'Cash Flow Statement',
  'wip-schedule': 'WIP Schedule',
  'job-cost-summary': 'Job Cost Summary',
  'job-cost-detail': 'Job Cost Detail',
  'aging-ap': 'AP Aging',
  'aging-ar': 'AR Aging',
  'payroll-summary': 'Payroll Summary',
  'payroll-detail': 'Payroll Detail',
  'equipment-utilization': 'Equipment Utilization',
  'equipment-cost': 'Equipment Cost',
  'bonding-capacity': 'Bonding Capacity',
  'custom': 'Custom',
};

const FINANCIAL_TYPES = new Set(['balance-sheet', 'income-statement', 'cash-flow']);
const CONSTRUCTION_TYPES = new Set(['wip-schedule', 'job-cost-summary', 'job-cost-detail']);

function getBadgeCls(reportType: string): string {
  if (FINANCIAL_TYPES.has(reportType)) {
    return 'px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20';
  }
  if (CONSTRUCTION_TYPES.has(reportType)) {
    return 'px-2 py-0.5 rounded-full text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20';
  }
  return 'px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
}

// ---------------------------------------------------------------------------
// Module
// ---------------------------------------------------------------------------

const wrapper = el('div', 'space-y-0');

function renderContent(): void {
  wrapper.innerHTML = '';

  // Header
  const headerRow = el('div', 'flex items-center justify-between mb-4');
  headerRow.appendChild(el('h1', 'text-2xl font-bold text-[var(--text)]', 'Report Templates'));

  const actionsRow = el('div', 'flex items-center gap-3');
  const newBtn = el('button', 'px-4 py-2 rounded-md text-sm font-medium bg-[var(--accent)] text-white hover:opacity-90', 'New Template');
  actionsRow.appendChild(newBtn);
  const backLink = el('a', 'text-sm text-[var(--accent)] hover:underline', 'Back to Reports') as HTMLAnchorElement;
  backLink.href = '#/reports';
  actionsRow.appendChild(backLink);
  headerRow.appendChild(actionsRow);
  wrapper.appendChild(headerRow);

  // Filter bar
  const filterBar = el('div', 'flex items-center gap-3 mb-4');
  const inputCls = 'bg-[var(--surface)] border border-[var(--border)] rounded-md px-3 py-2 text-sm text-[var(--text)]';
  const filterLabel = el('label', 'text-sm text-[var(--text-muted)]', 'Filter by type:');
  filterBar.appendChild(filterLabel);
  const filterSelect = el('select', inputCls) as HTMLSelectElement;
  const allOpt = el('option', '', 'All') as HTMLOptionElement;
  allOpt.value = '';
  filterSelect.appendChild(allOpt);
  for (const rt of REPORT_TYPES) {
    const o = el('option', '', REPORT_TYPE_LABELS[rt] ?? rt) as HTMLOptionElement;
    o.value = rt;
    filterSelect.appendChild(o);
  }
  filterBar.appendChild(filterSelect);
  wrapper.appendChild(filterBar);

  // Inline form container (hidden initially)
  const formContainer = el('div', 'hidden mb-4 p-4 bg-[var(--surface-raised)] border border-[var(--border)] rounded-lg');
  formContainer.appendChild(el('h3', 'text-sm font-semibold text-[var(--text)] mb-3', 'New Template'));

  const formGrid = el('div', 'grid grid-cols-1 md:grid-cols-2 gap-3 mb-3');

  const nameInput = el('input', inputCls) as HTMLInputElement;
  nameInput.type = 'text';
  nameInput.placeholder = 'Template name';
  formGrid.appendChild(nameInput);

  const typeSelect = el('select', inputCls) as HTMLSelectElement;
  for (const rt of REPORT_TYPES) {
    const o = el('option', '', REPORT_TYPE_LABELS[rt] ?? rt) as HTMLOptionElement;
    o.value = rt;
    typeSelect.appendChild(o);
  }
  formGrid.appendChild(typeSelect);

  formContainer.appendChild(formGrid);

  const descInput = el('input', inputCls + ' w-full mb-3') as HTMLInputElement;
  descInput.type = 'text';
  descInput.placeholder = 'Description (optional)';
  formContainer.appendChild(descInput);

  const defaultLabel = el('label', 'flex items-center gap-2 text-sm text-[var(--text-muted)] mb-3');
  const defaultCheck = el('input') as HTMLInputElement;
  defaultCheck.type = 'checkbox';
  defaultLabel.appendChild(defaultCheck);
  defaultLabel.appendChild(document.createTextNode('Set as default'));
  formContainer.appendChild(defaultLabel);

  const formBtns = el('div', 'flex items-center gap-2');
  const saveBtn = el('button', 'px-4 py-2 rounded-md text-sm font-medium bg-[var(--accent)] text-white hover:opacity-90', 'Save');
  const cancelBtn = el('button', 'px-4 py-2 rounded-md text-sm font-medium border border-[var(--border)] text-[var(--text)] hover:bg-[var(--surface)]', 'Cancel');
  formBtns.appendChild(saveBtn);
  formBtns.appendChild(cancelBtn);
  formContainer.appendChild(formBtns);

  wrapper.appendChild(formContainer);

  // Table container
  const tableContainer = el('div');
  wrapper.appendChild(tableContainer);

  // Toggle new template form
  newBtn.addEventListener('click', () => {
    formContainer.classList.toggle('hidden');
    if (!formContainer.classList.contains('hidden')) {
      nameInput.value = '';
      descInput.value = '';
      defaultCheck.checked = false;
      nameInput.focus();
    }
  });

  cancelBtn.addEventListener('click', () => {
    formContainer.classList.add('hidden');
  });

  // Save template
  saveBtn.addEventListener('click', () => {
    const name = nameInput.value.trim();
    if (!name) {
      showMsg(wrapper, 'Template name is required.', true);
      return;
    }

    void (async () => {
      try {
        const svc = getReportsService();
        await svc.createTemplate({
          name,
          reportType: typeSelect.value as typeof REPORT_TYPES[number],
          description: descInput.value.trim() || undefined,
          columns: [],
          filters: [],
          groupBy: [],
          sortBy: [],
          isDefault: defaultCheck.checked,
        });

        formContainer.classList.add('hidden');
        showMsg(wrapper, `Template "${name}" created successfully.`, false);
        await loadTemplates();
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : 'Operation failed';
        showMsg(wrapper, message, true);
      }
    })();
  });

  // Load and render templates
  async function loadTemplates(): Promise<void> {
    try {
      const svc = getReportsService();
      const filterType = filterSelect.value || undefined;
      const templates = await svc.getTemplates(filterType as typeof REPORT_TYPES[number] | undefined);

      tableContainer.innerHTML = '';

      const wrap = el('div', 'bg-[var(--surface-raised)] border border-[var(--border)] rounded-lg overflow-hidden');
      const table = el('table', 'w-full text-sm');

      const thead = el('thead');
      const headRow = el('tr', 'text-left text-[var(--text-muted)] border-b border-[var(--border)]');
      for (const col of ['Name', 'Report Type', 'Description', 'Columns', 'Default', 'Actions']) {
        headRow.appendChild(el('th', 'py-2 px-3 font-medium', col));
      }
      thead.appendChild(headRow);
      table.appendChild(thead);

      const tbody = el('tbody');
      if (templates.length === 0) {
        const tr = el('tr');
        const td = el('td', 'py-8 px-3 text-center text-[var(--text-muted)]', 'No report templates found. Create a template to save your report configurations.');
        td.setAttribute('colspan', '6');
        tr.appendChild(td);
        tbody.appendChild(tr);
      }

      for (const template of templates) {
        const tr = el('tr', 'border-b border-[var(--border)] hover:bg-[var(--surface)] transition-colors');

        tr.appendChild(el('td', 'py-2 px-3 font-medium text-[var(--text)]', template.name));

        const tdType = el('td', 'py-2 px-3');
        const badgeCls = getBadgeCls(template.reportType);
        const label = REPORT_TYPE_LABELS[template.reportType] ?? template.reportType;
        tdType.appendChild(el('span', badgeCls, label));
        tr.appendChild(tdType);

        tr.appendChild(el('td', 'py-2 px-3 text-[var(--text-muted)]', template.description || '-'));
        tr.appendChild(el('td', 'py-2 px-3 text-center', String(template.columns?.length ?? 0)));

        const tdDefault = el('td', 'py-2 px-3 text-center');
        if (template.isDefault) {
          tdDefault.appendChild(el('span', 'px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20', 'Default'));
        } else {
          tdDefault.appendChild(el('span', 'text-xs text-[var(--text-muted)]', '-'));
        }
        tr.appendChild(tdDefault);

        const tdActions = el('td', 'py-2 px-3');
        const actRow = el('div', 'flex items-center gap-2');

        const editBtn = el('button', 'text-[var(--text-muted)] hover:underline text-sm', 'Edit');
        editBtn.addEventListener('click', () => {
          const newName = prompt('Template name:', template.name);
          if (newName === null) return;
          const newDesc = prompt('Description:', template.description || '');
          if (newDesc === null) return;

          void (async () => {
            try {
              const svc = getReportsService();
              await svc.updateTemplate(template.id, {
                name: newName.trim() || template.name,
                description: newDesc.trim() || undefined,
              });
              showMsg(wrapper, 'Template updated successfully.', false);
              await loadTemplates();
            } catch (err: unknown) {
              const message = err instanceof Error ? err.message : 'Operation failed';
              showMsg(wrapper, message, true);
            }
          })();
        });
        actRow.appendChild(editBtn);

        const deleteBtn = el('button', 'text-red-400 hover:underline text-sm', 'Delete');
        deleteBtn.addEventListener('click', () => {
          if (!confirm(`Delete template "${template.name}"?`)) return;

          void (async () => {
            try {
              const svc = getReportsService();
              await svc.deleteTemplate(template.id);
              showMsg(wrapper, `Template "${template.name}" deleted.`, false);
              await loadTemplates();
            } catch (err: unknown) {
              const message = err instanceof Error ? err.message : 'Operation failed';
              showMsg(wrapper, message, true);
            }
          })();
        });
        actRow.appendChild(deleteBtn);

        tdActions.appendChild(actRow);
        tr.appendChild(tdActions);

        tbody.appendChild(tr);
      }

      table.appendChild(tbody);
      wrap.appendChild(table);
      tableContainer.appendChild(wrap);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Operation failed';
      showMsg(wrapper, message, true);
    }
  }

  // Filter change handler
  filterSelect.addEventListener('change', () => {
    void loadTemplates();
  });

  // Initial load
  void loadTemplates();
}

void (async () => {
  try {
    const svc = getReportsService();
    void svc;
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Operation failed';
    showMsg(wrapper, message, true);
  }
})();

export default {
  render(container: HTMLElement): void {
    container.innerHTML = '';
    renderContent();
    container.appendChild(wrapper);
  },
};
