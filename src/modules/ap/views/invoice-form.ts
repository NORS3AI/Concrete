/**
 * AP Invoice create/edit form view.
 * Full invoice details with line items for creating or editing an AP invoice.
 */
...
function buildLineItemsTable(lines: InvoiceLine[]): HTMLElement {
  ...
  for (const col of ['#', 'Description', 'Job', 'Cost Code', 'Cost Type', 'GL Account', 'Amount', 'Retention', 'Net', 'Actions']) {
    const align = ['Amount', 'Retention', 'Net'].includes(col)
  ...
  for (const line of lines) {
    ...
    tr.appendChild(el('td', 'py-2 px-3 font-mono', line.jobNumber));
    ...
    tr.appendChild(el('td', 'py-2 px-3 text-right font-mono text-[var(--text-muted)]', fmtCurrency(line.retentionAmount)));
    tr.appendChild(el('td', 'py-2 px-3 text-right font-mono font-medium', fmtCurrency(line.netAmount)));
...
function buildTotals(): HTMLElement {
  ...
  grid.appendChild(buildTotal('Retention', fmtCurrency(0)));
  grid.appendChild(buildTotal('Tax', fmtCurrency(0)));
  grid.appendChild(buildTotal('Total Due', fmtCurrency(0)));
...
export default {
  render(container: HTMLElement): void {
    ...
    headerGrid.appendChild(buildField('Invoice Number', textInput('invoiceNumber', 'Vendor invoice number')));
    ...
    headerGrid.appendChild(buildField('PO Number', textInput('poNumber', 'Purchase order reference')));
    headerGrid.appendChild(buildField('Status', selectInput('status', STATUS_OPTIONS)));
    headerGrid.appendChild(buildField('Retention %', numberInput('retentionPct', '0')));
    ...
    approvalGrid.appendChild(buildField('Approval Date', dateInput('approvalDate')));
    approvalGrid.appendChild(buildField('Approval Notes', textareaInput('approvalNotes', 2), 2));
