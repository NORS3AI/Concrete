 ...
 * Vendor aging report with aging buckets (Current, 1-30, 31-60, 61-90, 90+).
 */
...
const AGING_COLUMNS = ['Current', '1-30', '31-60', '61-90', '90+', 'Total'];
...
function buildSummaryCards(): HTMLElement {
  const row = el('div', 'grid grid-cols-6 gap-3 mb-4');
  ...
  row.appendChild(buildCard('90+ Days', fmtCurrency(0), 'text-red-400'));
...
function buildTable(rows: AgingRow[]): HTMLElement {
  ...
  if (rows.length === 0) {
    const tr = el('tr');
    const td = el('td', 'py-8 px-3 text-center text-[var(--text-muted)]', 'No aging data available. Enter AP invoices to generate the aging report.');
  ...
  for (const row of rows) {
    ...
    tr.appendChild(el('td', amountCls(row.days90plus, true), fmtCurrency(row.days90plus)));
...
export default {
  render(container: HTMLElement): void {
    ...
    headerRow.appendChild(el('h1', 'text-2xl font-bold text-[var(--text)]', 'AP Aging Report'));
